...
const PI_WEB_ACCESS_FIXTURE_ROOT = join(import.meta.dirname, "fixtures", "pi-web-access-0.23.0");
function writePiWebAccessFixture(webRoot: string, version = "0.23.0", patched = false): void {
...
test("patchPiRuntimeNodeModules rejects unsupported pi-web-access versions before launch", async () => {
	...
	writePiWebAccessFixture(webRoot, "0.24.0");
...
test("patchPiRuntimeNodeModules patches the Windows npm prefix layout", () => {
	...
	writePiWebAccessFixture(webRoot, "0.23.0", true);
	writeFileSync(
		webAccessPath,
